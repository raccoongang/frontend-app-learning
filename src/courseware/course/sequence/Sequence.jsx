...
function Sequence({
  ...
  */
  if (sequenceStatus === 'loaded') {
    const isDisabledSpecialExams = sequence.isTimeLimited && !specialExamsEnabledWaffleFlag;
    const isDisabledProctoredExams = sequence.isProctored && specialExamsEnabledWaffleFlag
      && !proctoredExamsEnabledWaffleFlag;

    if (isDisabledSpecialExams || isDisabledProctoredExams) {
      return (
        <PageLoading
          srMessage={intl.formatMessage(messages['learn.loading.learning.sequence'])}
        />
