  ...
  return (
    sectionScores.map((chapter) => {
      const subsectionScores = chapter.subsections.filter(
        (subsection) => !!(subsection.hasGradedAssignment && subsection.showGrades && subsection.numPointsPossible > 0),
      );
      ...
      const detailedGradesData = subsectionScores.map((subsection) => {
        const title = <a href={subsection.url} className="text-dark-700">{subsection.displayName}</a>;
        return {
