...
function GradeSummaryTable({
  ...
  const getFootnoteId = (assignment) => (
    `${assignment.shortLabel ? assignment.shortLabel : (assignment.type).replace(/\s+/g, '-')}`
  );
  ...
  const gradeSummaryData = assignmentPolicies.map((assignment) => {
    ...
    if (assignment.numDroppable > 0) {
      ...
      footnoteMarker = asterisk.repeat(footnotes.length);
    }
  ...
  return (
    <>
      ...
      {footnotes && (
        <DroppableAssignmentFootnote footnotes={footnotes} marker={asterisk} />
      )}
    </>
  );
}
